...
#     # Установка входных и выходных переменных
...
#     x_unloading = np.arange(0, 101, 1)
#     x_loading = np.arange(0, 101, 1)
#     x_waiting = np.arange(0, 101, 1)
#     x_quality = np.arange(0, 101, 1)
#
#     # Членства
#     unloading_low = fuzz.trimf(x_unloading, [0, 0, 50])
#     unloading_medium = fuzz.trimf(x_unloading, [0, 50, 100])
#     unloading_high = fuzz.trimf(x_unloading, [50, 100, 100])
#
#     loading_low = fuzz.trimf(x_loading, [0, 0, 50])
#     loading_medium = fuzz.trimf(x_loading, [0, 50, 100])
#     loading_high = fuzz.trimf(x_loading, [50, 100, 100])
#
#     waiting_low = fuzz.trimf(x_waiting, [0, 0, 50])
#     waiting_medium = fuzz.trimf(x_waiting, [0, 50, 100])
#     waiting_high = fuzz.trimf(x_waiting, [50, 100, 100])
#
#     quality_good = fuzz.trimf(x_quality, [0, 0, 50])
#     quality_average = fuzz.trimf(x_quality, [0, 50, 100])
#     quality_poor = fuzz.trimf(x_quality, [50, 100, 100])
#
#     # Применение нечетких правил
#     for i in range(len(df)):
#         unloading_lvl = fuzz.interp_membership(x_unloading, unloading_low, unloading_time[i])
#         loading_lvl = fuzz.interp_membership(x_loading, loading_low, loading_time[i])
#         waiting_lvl = fuzz.interp_membership(x_waiting, waiting_low, waiting_time[i])
#
...
#
#         # Находим конечное качество
#         aggregated_quality = np.fmax(quality_activation_good, quality_activation_average)
#         quality_result = fuzz.defuzz(x_quality, aggregated_quality, 'centroid')
#
#         df.at[i, 'Quality Assessment'] = quality_result
#
